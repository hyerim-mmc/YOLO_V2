...
def yolo_loss(output_pred, ground_truth, height, width):
    ...
    for i in range(batch_size):
        ...
        anchors_gt_iou = box_overlaps(anchors_xyxy, gt_boxes_xyxy).view(
            -1, anchor_num, gt_num
        )  # decide which anchor is responsible for the gt_box
    ...
    coord_loss = (
        cfg.COORD_SCALE
        * F.mse_loss(coord_pred_variable * coord_mask, coord_target * coord_mask, reduction="sum")
        / batch_size
        / 2.0
    )
    conf_loss = F.mse_loss(conf_pred_variable * conf_mask, conf_target * conf_mask, reduction="sum") / batch_size / 2.0
